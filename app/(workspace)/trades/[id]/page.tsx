...
export default async function TradePage({
  ...
  // Started here and awaited at the end, so it travels with the first paint
  // instead of arriving after mount. The card row sits directly above the accept
  // controls, and it
  // was growing from one line of "Checking your card…" into a bordered row a
  ...
  if (trade.state === 'COMPLETED') {
    ...
    // Just the control. The heading and the "share how it went with X" line are
    // gone with the strip that carried them: the action card states the step in
    // its own title, so a second card underneath repeating it was one banner too
    // many at exactly the moment the contract is over.
    reviewSection = existingReview ? (
      <span className="text-body text-muted-foreground">Reviewed</span>
    ) : (
      <LeaveReviewDialog
  ...
  return (
    <MarketplaceShell
      title="Trade"
      // ALWAYS FLUSH — see the matching note on the cash sale route. The
      // completed-state exception existed for the review strip that used to hang
      // below the room; it lives in the action card now.
      flush
    >
      <TradeContract
        tradeId={trade.id}
