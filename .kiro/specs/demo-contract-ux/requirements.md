...
3. The Mock_Service and PinchService implementations SHALL agree with the UI’s chosen collateral model. If the model is a later-charge authorization, the demo SHALL show an authorization/charge exposure, not a captured charge, refund, or claimed provider pre-auth.
