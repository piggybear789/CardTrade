...
export default async function CashSalePage({
  ...
  return (
    ...
    <MarketplaceShell
      ...
      // ALWAYS FLUSH. This was `status !== 'COMPLETED'`, because a completed sale
      // appended a "rate this transaction" strip below the room and the page had
      // to scroll to reach it. The review moved into the action card, so nothing
      // renders below the room any more — and leaving the exception in gave a
      // finished contract a document scrollbar with nothing under the fold.
      flush
    >
      <CashSaleView
        initialSale={sale}
        ...
        reviewAction={
          sale.status !== 'COMPLETED' ? null : existingReview ? (
