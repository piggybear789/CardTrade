...
2. **2-Way Trade Escrow** — Two users swap goods of equal Fair Market Value with $0 cash. A pre-authorization hold for 100% of FMV is placed on both parties and voided once both receive and accept.
   ...
   **UNRESOLVED: the Identity_Gate and the Bond Policy currently cancel each other out for trades.** `proposeTradeAction` gates BOTH parties on the Identity_Gate (Req 14.2, because a fraud finding can pay either side), and `resolveTradeBonds` exempts a verified trader. Every trade that can legally start therefore posts **zero collateral**, `bondsRequired === 0`, and no `pre_auth_holds` row is ever written — so a Condition_Dispute has no $20 to partial-capture and an Objective_Fraud finding pays the victim nothing. The dispute and fraud branches of the state machine are unreachable in production as written. Pick one: drop the gate for the party who is only *giving* (and gate the payout instead, at resolution time), or stop exempting verified traders from a trade bond. The demo fixture in `supabase/seeds/demo_lifecycle.sql` sidesteps this by making one trader `PENDING`, which is exactly the configuration the gate forbids.
